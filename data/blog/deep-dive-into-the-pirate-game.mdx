...
{/* TODO add italics and bold text */}

# Motif

I came across this problem first in high school. At that time, I found it as an eye-opening introductory problem to
game theory. And I find the solution incredibly counter-intuitive yet elegant.

Recently, I got asked this exact problem in an interview for a developer job. Thank to my high school experience,
I nailed the solution and impressed the interviewer.

This brings me to share this exicting game-theory problem with you. Buckle up!

# The Game

Amaro is the captain of a pirate ship. His mateys, Bart, Charlotte, Daniel, and Eliza, are the other
...
The code stipulates that Amaro, as captain, gets to suggest the first plan for distributing the coins
among the five pirates. For example, 40 for Amaro, 30 for Bart, 20 for Charlotte, 10 for Daniel and 0 for Eliza. (Sorry, Eliza).

After that proposal, each pirate (including Amaro) votes "yarr" or nay on
...
meet this threshold, Amaro must walk the plank (for people who are not very familiar with pirate folklore like me, this means death), making Bart the next captain.
...
Each of the pirates act in a _SELFISH_ yet _EXTREMELY RATIONAL_ way, their action obeys the following rules:
...
So we arrive at the key problem for Amaro:

**What distribution should he propose to ensure he lives and maximizes his own gold return?**

_Feel free to stop scrolling down and ponder your strategy for a bit._

# Solution

For simplicity, we will address the pirates as A, B, C, D and E.

## Observations

According to the Rule #1 in the Pirates' Rational, A wants to gurantee that his proposal gets passed at all cost.

For example, we can consider the proposal where A leaves no coin to himself and gives each other pirate 25 coins.
I would give a round of applause for the generosity behind such thinking. However, in this case, the gold gain of Amaro
is not maximised and the problem is still far from being solved.

In fact, in order to survive, A needs only 2 "yarrs" from other pirates (with a vote from A himself, 3 votes makes the majority for 5 pirates).
So he can probably give coins to only 2 other pirates.

Therefore, the problem becomes, what is the minimum amount of coins required for each of B, C, D and E to vote "yarrs"?
If we can answer this question, Amaro would bribe two of them with the least needs.
...
Because every pirate acts rationally and want to maximise their gain, they will only
agree the proposal of A if and only if A is giving them more gold than how much B would give.

Figuring out B's optimal proposal has no difference from figuring out A's except there is only
4 pirates left. Similarly, in the process of resolving B's proposal, we will discover it is not possible
without knowing the proposal C would make.

In other words, we should know how the captain of n pirates would propose, before deducing the reactions of
each pirate towards the captain of n+1 pirates.

Let the logic above sink for a bit. And we shall explore what each pirate exactly thinks in each scenario.

## Work Backwards

### E as the captain

As the only person on the vessel, E would give all 100 coins to herself. Unless E has a sudden moment
of philosophical englightment that life is all meaningless, the proposal would pass.

However, we will soon learn that E will never become the captain.

### D as the captain

In this case, there are 2 pirates only, it is not hard to see D can force a tie no matter what he proposes.
Therefore, he would give 100 coins to himself, and E would reject, but the proposal gets passed anyway.

Notice how when D is the captain, E gets absolutely nothing. This is a base for further analysis.

### C as the captain

Now the fun begins, as the captain, C needs another crew to vote for her to win the
...
This situtaion is similar to the previous one with C as the captain. As there are 4 pirates, B needs a vote from another crew to force a tie.

Everyone would recognize that if the proposal gets rejected, C would get 99 coins and E would get 1.
Because the pirates are bloodthirsty, had B given E 1 coin, E would not be satisfactory as she can get the same amount from C anyway.
...
Now we have all the information to resolve the case for Amaro.
...
A will propose keeping 98 coins to herself, 0 to B, 1 to C, 0 to D and 1 to E.
...
It is not hard to notice the pattern when there is more than 5 pirates.
The captain would give a coin to every other crew, and keep the rest to themselves.
...
We will use a slightly different annotation for this section. Suppose there are $n$ pirates on the vessel, we will call them #$n$, #$n-1$, ..., #1 with decreasing captain priority. That is, #n will be the first captain.
...
Similarly, Pirate #202 can survive by giving 1 coin to every pirate with even number.
...
to the 101 votes #203 can get. Very importantly, he can get a free-of-charge vote from #203! Because #203 knows, if #204
gets voted off, she would not be able to survive either as the captain.

What happends afterwards?
...
So far, we learnt #201, #202, #204, #208 and #216 are capable of making their proposal pass. Do you notice a pattern here?
...
Any pirate who does not have survival issue will reject the proposal out of their bloodthirstiness. The pirates with highest
captain prioirties will deseprately vote for the incumbent captain to keep themselves alive. Since half of the crew's votes
is needed for the proposal to pass. Eventually, there will be enough number of new-joiners to protect a new captain. Afterwards,
...
the voluntary voting crew will double in size, hence the power of two.

## Determine the Bribable Pirates

In the 5-pirate scenario, we discussed the pattern: odd-numbered captains would bribe odd-numbered crews and even-numbered
captain would bribe even-numbered crew. Let's investigate whether this still holds for 200+ pirates.

#201 has to give coins to odd-numbered pirates, because all the even-numbered pirates would receive a coin from #200.

#202 is in a pretty similar spot, she also needs 101 votes to survive. The odd-numbered pirates in the range #1-#200 is harder
...
Therefore, #201 is also willing to say "yarr" for the price of 1 coin. #202 can choose 100 pirates out of the 101 candidates and give 1 coin each.

Following this train of thinking, one would argue #204 has to bribe odd-numbered crews. But now he has a lot more freedom. In fact,
his candidates are everybody in the range #1-#202. Because #202 had the freedom to pick 100 pirates out of 101.
...
Suppose $N$ pirates are splitting $G$ gold coins among them.

## Scenario I: $N \le 2G + 2$
...
- Pirates up to $2G + M$ can survive. Where M is the greatest power of 2 up to N - 2G.
  {/* - i.e. if there are 100 coins, pirates 204, 208, 216, ... are special pirates that can survive as captain */}
- The captain will not get any coins.
- Pirates who are _bribable_ has a chance of $\frac{G}{|bribable|}$ to get 1 coin. For a captain numbered $C$, the bribable pirates are as follows.
...
The elegance of the Pirate Game stroke me as counter-intuitive because I did not expect
Amaro to get as much as 98 coins for herself. This is all possible because they know
every pirate is so rational and emotionless that the gain of 1 coin would outweight the frustration and anger
of such a minuscule amount.

After all, the Pirate Game is just a game theory exercise which, like many other game-theory problems, does not necessarily capture the real-world behaviours.
The overlooked human emotion often times is the deciding factor in similar scenarios.

On a side note, this problem is actually a decent choice for software engineer interviews.
because the solution nicely captures the concept of recursion.
